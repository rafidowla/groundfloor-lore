 ...
 * invalidation races hide. One fresh SurrealGraph workspace per test block.
 *
 ...
 *
 * Airplane-safe: all local SurrealDB, no network.
 */
...
async function withGraph<T>(fn: (g: SurrealGraph) => Promise<T>): Promise<T> {
    ...
    const g = new SurrealGraph(ws);
    await g.initialize();
    try { return await fn(g); }
    finally {
        try { await g.close(); } catch { /* best-effort */ }
...
// analog exists for this scenario.
