 ...
 *     Removed in X5 (2026-05-22): the developer plugin (which was the
 *     fixture for this case) was deleted along with Atlas being moved
 *     to its own repo. Default plugin set is now empty; no built-in
 *     plugin provisions cloud collections by default. Re-add this case
 *     when a future plugin lands with `registerCloudSchema`.
 *
...
async function main(): Promise<void> {
    ...
    try {
        ...
        // Case E (plugin cloud-schema provisioning) removed in X5 —
        // see the file header for context. Reinstate when a follow-on
        // plugin with `registerCloudSchema` lands.
