...
const tests: Array<() => Promise<void>> = [
    ...
    async () => {
        // RA2-reaudit2 — a non-ok response must NOT leak the endpoint URL or the
        // raw upstream body in the thrown error (it can reach an HTTP client).
        const { fetch: fakeFetch } = makeFetch(() => new Response('SECRET upstream detail', { status: 500 }));
        const provider = new OpenAICompatEmbeddingProvider({
            baseUrl: 'https://internal-gw.example/v1',
            modelId: 'bge-m3',
            dimension: 4,
            fetchImpl: fakeFetch,
        });
        let msg = '';
        try { await provider.embed('x'); } catch (e) { msg = (e as Error).message; }
        assert.ok(msg.includes('HTTP 500'), 'error still reports the status');
        assert.ok(!msg.includes('internal-gw.example'), 'endpoint URL must not leak');
        assert.ok(!msg.includes('SECRET upstream detail'), 'upstream body must not leak');
        console.log('  ok  non-ok error redacts endpoint + upstream body');
    },

    async () => {
        // RA2-reaudit2 — embedDocumentBatch must order vectors by the response
        // `index` (OpenAI-compatible servers may return them out of order), not
        // by arrival order.
        const E0 = [0, 0, 0, 0]; const E1 = [1, 1, 1, 1];
        const { fetch: fakeFetch } = makeFetch(() =>
            jsonResponse({ data: [{ index: 1, embedding: E1 }, { index: 0, embedding: E0 }] })
        );
        const provider = new OpenAICompatEmbeddingProvider({
            baseUrl: 'https://example.test/v1', modelId: 'm', dimension: 4, fetchImpl: fakeFetch,
        });
        const vecs = await provider.embedDocumentBatch(['a', 'b']);
        assert.deepEqual(vecs[0], E0, 'vec[0] matches index 0 (input a)');
        assert.deepEqual(vecs[1], E1, 'vec[1] matches index 1 (input b)');
        console.log('  ok  embedDocumentBatch orders vectors by response index');
    },
    ...
    async () => {
        // RA2-reaudit2 — HTTP non-2xx surfaces as a typed error reporting the
        // STATUS, but NOT the upstream body excerpt (was leaked pre-fix).
        const { fetch: fakeFetch } = makeFetch(() =>
        ...
        await assert.rejects(
            ...
            (err: unknown) => {
                ...
                assert.ok(!(err as Error).message.includes('rate limit exceeded'), 'upstream body must not leak');
                return true;
            }
        );
