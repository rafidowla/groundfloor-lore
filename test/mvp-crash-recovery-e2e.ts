 ...
 * lost — graph nodes persisted (Kùzu durability) AND the un-drained embed rows
...
const N = 6; // single synchronous writes (default embed) — land in Kùzu before each 200
...
async function main(): Promise<void> {
    ...
    try {
        ...
        // is written to Kùzu SYNCHRONOUSLY (recorded to graph.wal) before its 200,
        ...
        // process — not just the npx wrapper — dies and releases Kùzu's OS-level
        ...
        // recorded to Kùzu's graph.wal before each 200, so reopening the data dir
