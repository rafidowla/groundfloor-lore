/**
 * NodeDetailDrawer — Slides in from the right when the user clicks a
 * graph node. Shows the node's content + its immediate neighbors; the
 * "Ask about this" button fills the chat input with a [node:id] marker
 * the server expands into system-prompt context.
 */

import { useEffect, useState } from 'react';
import { X, MessageSquare } from 'lucide-react';

interface NodeDetail {
    node: {
        id: string;
        label: string;
        type: string;
        content?: string;
        tags?: string;
        project?: string;
        ecosystem?: string;
        updatedAt?: string;
    };
    neighbors: Array<{
        id: string;
        label: string;
        type: string;
        relation: string;
        depth: number;
    }>;
}

interface NodeDetailDrawerProps {
    apiBase: string;
    selectedNodeId: string | null;
    onClose: () => void;
    onAskAbout: (nodeId: string) => void;
}

export default function NodeDetailDrawer({
    apiBase,
    selectedNodeId,
    onClose,
    onAskAbout,
}: NodeDetailDrawerProps) {
    const [detail, setDetail] = useState<NodeDetail | null>(null);
    const [error, setError] = useState<string | null>(null);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (!selectedNodeId) {
            setDetail(null);
            setError(null);
            return;
        }
        // Plugin-owned nodes (file:, symbol:) don't have a GET /api/node
        // endpoint yet — skip the fetch and show a minimal card.
        const isCore = selectedNodeId.startsWith('lore:') || !selectedNodeId.includes(':');
        if (!isCore) {
            setDetail({
                node: {
                    id: selectedNodeId,
                    label: selectedNodeId.split(':').slice(1).join(':'),
                    type: selectedNodeId.split(':')[0],
                    content: '(Plugin-owned node — detail endpoint not yet implemented for this kind.)',
                },
                neighbors: [],
            });
            return;
        }
        setLoading(true);
        setError(null);
        void fetch(`${apiBase}/api/node?id=${encodeURIComponent(selectedNodeId)}`)
            .then(async (r) => {
                if (!r.ok) throw new Error(`HTTP ${r.status}`);
                return r.json() as Promise<NodeDetail>;
            })
            .then(setDetail)
            .catch((e: Error) => setError(e.message))
            .finally(() => setLoading(false));
    }, [apiBase, selectedNodeId]);

    if (!selectedNodeId) return null;

    return (
        <div className="node-drawer glass-panel">
            <header className="node-drawer-header">
                <div className="node-drawer-title">
                    {detail ? (
                        <>
                            <span className={`node-type-badge type-${detail.node.type}`}>
                                {detail.node.type}
                            </span>
                            <h3>{detail.node.label}</h3>
                        </>
                    ) : (
                        <h3>{loading ? 'Loading…' : selectedNodeId}</h3>
                    )}
                </div>
                <button className="icon-button" onClick={onClose} title="Close">
                    <X size={18} />
                </button>
            </header>

            {error ? (
                <div className="node-drawer-error">Failed to load: {error}</div>
            ) : detail ? (
                <div className="node-drawer-body">
                    <div className="node-drawer-meta">
                        <code>{detail.node.id}</code>
                        {detail.node.project ? <span>project: {detail.node.project}</span> : null}
                        {detail.node.tags ? <span>tags: {detail.node.tags}</span> : null}
                    </div>

                    {detail.node.content ? (
                        <section>
                            <h4>Content</h4>
                            <p className="node-drawer-content">{detail.node.content}</p>
                        </section>
                    ) : null}

                    {detail.neighbors.length > 0 ? (
                        <section>
                            <h4>Connected to ({detail.neighbors.length})</h4>
                            <ul className="node-drawer-neighbors">
                                {detail.neighbors.map((n) => (
                                    <li key={n.id + n.relation}>
                                        <span className="rel-label">{n.relation}</span>
                                        <span className="neighbor-type">{n.type}</span>
                                        <span className="neighbor-label">{n.label}</span>
                                    </li>
                                ))}
                            </ul>
                        </section>
                    ) : (
                        <section>
                            <h4>Connected to</h4>
                            <p className="help-text">
                                No edges yet. Run <code>lore reconsume</code> to lay
                                semantic links to similar nodes.
                            </p>
                        </section>
                    )}

                    <button
                        className="node-drawer-ask"
                        onClick={() => onAskAbout(detail.node.id)}
                        title="Pre-fill the chat with [node:id] so the LLM answers in context"
                    >
                        <MessageSquare size={14} />
                        Ask about this in chat
                    </button>
                </div>
            ) : null}
        </div>
    );
}
