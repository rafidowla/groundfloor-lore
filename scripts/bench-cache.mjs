...
const { LocalGraph } = await import(path.join(repoRoot, 'dist/lore/src/engines/localGraph.js'));
...
async function main() {
    ...
    const cold = new LocalGraph(BASE_PATH);
    await seed(cold, 100);
    const coldSamples = await runWorkload(cold, ITERATIONS);

    // Phase 2: hot (cached). Use a fresh LocalGraph on the same DB
    // so Kùzu's own internal buffer pool isn't pre-warmed unfairly in
    // the cache's favor. We're measuring our cache against Kùzu
    // operating normally.
    process.env.LORE_CACHE_DISABLED = '';
    delete process.env.LORE_CACHE_DISABLED;
    const hot = new LocalGraph(BASE_PATH);
