...
import { extractPdfTextItems, detectPdfTables, type PdfTextItem } from './pdfTables.js';
    ...
    async extract(input: Buffer, mimeType: string): Promise<ExtractedContent> {
        ...
        try {
            for (let pageNum = 1; pageNum <= doc.numPages; pageNum++) {
                ...
                // Bucket C — keep the positional items for table detection.
                allTextItems.push(...extractPdfTextItems(content));
        ...
        // Bucket C — geometric table detection (heuristic, best-effort).
        // A non-tabular layout yields [] and the flat text path is unchanged.
        ...
        const isSparse = !isScanned && charsPerPage < 50;

        return {
            // R3-DOS-02 — cap the text field to the 10 MB extracted-text budget.
            // joinedText (uncapped) still drives the metadata heuristics below.
            text: capText(joinedText),
            metadata: {
            ...
            quality: isScanned
                ? {
                    ...
                    upgradeMessage: `Scanned PDF — no text layer found across ${doc.numPages} page(s). A vision model will extract text from the images.`,
                }
                : isSparse
                ? {
                    reliable: false,
                    reason: 'sparse_pdf',
                    suggestedUpgrade: 'either',
                    upgradeMessage: `PDF text is very thin (${Math.round(charsPerPage)} chars/page). The document may contain images or complex layout that a vision model can read.`,
                }
                : undefined,
        };
    },
