 ...
 *
 * Composes the four storage surfaces for local mode (graph engine + LanceDB
 * + SQLite):
 *   - graph       → CollectionStorage over the local graph engine
 *   - verbatim    → VerbatimStoreAdapter wrapping VerbatimStore
 *   - analytical  → IAnalyticalStorage (see analyticalStorageFactory.ts)
 *   - tables      → ITableStorage (or STUB that throws if `tables`
 ...
 * graph connection + VerbatimStore + table-resolver, then pass them in.
...
/**
 * Stub ITableStorage that throws on every method. Used when the caller
 * doesn't supply a table store. Intentional throw —
 * silent no-ops would mask caller bugs.
 */
function unimplementedTables(): ITableStorage {
    const fail = <T>(op: string): Promise<T> =>
        Promise.reject(new Error(
            `LocalAdapter.tables.${op}: no table storage was supplied to this adapter. ` +
