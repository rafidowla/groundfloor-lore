...
const REGISTERED_EMBEDDERS: Record<string, EmbedderConfig> = {
    'local-xenova-multilingual': {
        ...
        // v1.1: corrected label — transformers.js v4 on Node routes
        // through onnxruntime-node natively, not Wasm. The exact EP
        // (CPU vs CoreML) is surfaced via /health.embeddingBackend.
        label: 'Xenova multilingual-e5-small (onnxruntime-node CPU; default for local mode)',
    },
    'local-xenova-minilm': {
        kind: 'local',
        modelId: MINILM_L6_V2_MODEL_ID,
        dimension: 384,
        label: 'Xenova all-MiniLM-L6-v2 (English-only; onnxruntime-node CPU)',
    },
};
...
export async function embedderCheckCommand(_args: string[]): Promise<void> {
    ...
    if (foundFast) {
        ...
        // v1.1 (deferred item #3): the Node entry point of @huggingface/
        // transformers v4 routes through onnxruntime-node native C++.
        // It is *not* the browser-side Wasm build despite the legacy
        // banner text some callers may have seen.
        console.log('No external accelerators detected. Lore is using onnxruntime-node (native C++)');
        console.log('with the bundled Xenova model files — CPU execution provider.');
        console.log('To get Metal/CUDA acceleration on top of that, install one of:');
...
export async function embedderSwitchCommand(args: string[]): Promise<void> {
    ...
    if (!apply) {
        ...
        console.log('  3. Re-embed every LoreNode with the new model');
