...
export async function pickEmbeddingProvider(workspaceBasePath?: string): Promise<PickResult> {
    ...
    // 3. Fallback — local Xenova model files via @huggingface/transformers
    //    v4. The Node.js entry point routes through onnxruntime-node (native
    //    C++) — has done since the slice-7 transformers upgrade. Use the
    //    runtime probe (providers/embeddingBackend.ts) to surface the actual
    //    ORT version + execution provider on /health.
    const provider = new LocalEmbeddingProvider();
    return {
        provider,
        banner: `[Lore] Embedder: local Xenova ${provider.modelId} (${provider.dimension}-dim) via onnxruntime-node (CPU). For Metal/CUDA acceleration install Ollama + 'ollama pull nomic-embed-text'.`,
        isFallback: true,
    };
}
