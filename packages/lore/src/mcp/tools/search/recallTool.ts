...
export function registerRecallTool(mcpServer: McpServer, deps: SearchToolsDeps): void {
    mcpServer.tool(
        ...
        'High-level knowledge recall: searches for a topic and traverses related nodes. Defaults to a compact summary (top hits, label + 1-line snippet) so the response stays under ~2KB and AI agents do not blow their context window. Pass mode="full" for the rich JSON, or use the get_full tool to fetch one node\'s body by id.',
        {
            ...
            types: z.array(z.string().max(100)).max(20).optional().describe('D2: keep only nodes whose `type` is one of these values (ANY match). A PREFILTER applied inside the vector + BM25 query itself, not after ranking — safe even when other node types crowd the candidate window. Only applies to a NAMED workspace. (E2: `project` is also pushed into the vector/BM25 and keyword queries; `entities`/`topics` into the keyword query only. `tags` is post-ranking only.)'),
        ...
        async ({ topic, depth, queryLanguage, filePaths, mode, crossProject, includeSuperseded, tags, queries, entities, topics, project, types, workspace, ecosystem, max_tokens, include_archived, search_mode, compact, abstain, relevance_floor }) => {
            try {
