...
import { readBoundedBody, isPayloadTooLarge, writeOversizeError, writeError, writeWorkspaceRequired } from '../helpers.js';
                    ...
                    if (hard_delete) {
                        ...
                        try {
                            ...
                            if (typeof vstore.tombstone === 'function') {
                                await vstore.tombstone(`lore:${node.id}`, 'graph node hard-deleted via /api/nodes/prune');
                            } else if (typeof vstore.delete === 'function') {
                                await vstore.delete(`lore:${node.id}`);
                            }
                        } catch (vErr) {
                            console.error(`[Lore HTTP] prune verbatim tombstone failed for ${node.id}: ${(vErr as Error).message}`);
                        }
                        hardDeleted++;
                    } else {
