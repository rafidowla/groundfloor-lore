...
export async function tryPolicyRoutes(req: IncomingMessage, res: ServerResponse, deps: RetentionDeps, pathname: string): Promise<boolean> {
    ...
    if (pathname === '/api/mark-stale' && req.method === 'POST') {
        ...
        try {
            ...
            const marked = await graph.markStaleByTags(parsed.tags);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ marked, tags: parsed.tags, ok: true }));
        } catch (msErr) {
    ...
    if (pathname === '/api/prune-ephemeral' && req.method === 'POST') {
        ...
        try {
            ...
            // (Kùzu, SurrealDB) and DataplaneGraph implements pruneEphemeralNodes
            // natively, so the prior 501-fallback guard is no longer needed.
