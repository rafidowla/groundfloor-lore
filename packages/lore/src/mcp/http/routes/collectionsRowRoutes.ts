/**
 * collectionsRowRoutes.ts — shared request-gating/routing plumbing for the
 * SDK-aligned tabular CRUD surface, plus the per-row `/v1/{collection}/{id?}`
 * dispatch half of it.
 *
 * Split out of collections.ts (which had grown to exactly the 800-line
 * hard cap) along its existing seam: collections.ts keeps the module
 * docstring/URL reference, `tryCollectionsRoutes` (the public entry point
 * the HTTP dispatcher calls), and the transaction / join-query / schema
 * (`/v1/schema*`) routes; everything here — `CollectionsRoutesDeps`, the
 * error classifier, the read/write ReBAC + workspace-routing gates, and
 * every route keyed on a `{collection}` path segment (single-row get/
 * insert/update/delete, the Phase 2.5 bulk/count/update-by-query/
 * delete-by-query/truncate variants) — lives here and is imported BY
 * collections.ts (one-way: collections.ts depends on this file, not the
 * reverse, so the split introduces no import cycle). `tryCollectionsRoutes`
 * calls `tryCollectionsRowRoutes` as its LAST step, once none of its own
 * routes matched — this is a pure extraction, route registration order is
 * unchanged and no route's behavior changed.
 *
 * See collections.ts's module docstring for the full URL reference and the
 * SDK-parity rationale.
 */
...
// Used by collections.ts (its own `/v1/schema*` routes) and by
// tryCollectionsRowRoutes below to compute `tail`.
export const V1_PREFIX = '/v1/';

/**
 * classifyStorageErr — Map a thrown storage error to an HTTP envelope.
 *
 * Used by every /v1/{collection}/... catch block (both here and in
 * collections.ts) so the daemon returns a clean 4xx (not 500) when the
 * request is structurally fine but the target collection or row is missing
 * or conflicting. The default still surfaces the op-specific `opCode` and
 ...
 */
export function classifyStorageErr(
    e: Error,
    opCode: string,
 ...
 */
export async function denyCollectionWrite(res: ServerResponse, deps?: CollectionsRoutesDeps): Promise<boolean> {
 ...
 */
export async function denyCollectionRead(res: ServerResponse, deps?: CollectionsRoutesDeps): Promise<boolean> {
 ...
 */
export async function routeDeps(
    deps: CollectionsRoutesDeps,
    res: ServerResponse,
...
/**
 * tryCollectionsRowRoutes — the `/v1/{collection}/{id?}` segment-split
 * dispatch. ALWAYS returns `true` (every path under here either matches a
 * route or falls through to the 405 at the bottom) — callers don't need to
 * re-check a prefix, `tryCollectionsRoutes` (collections.ts) already did
 * that.
 */
export async function tryCollectionsRowRoutes(
    req: IncomingMessage,
    res: ServerResponse,
    url: string,
    pathname: string,
    ...
    // routes in collections.ts, or reserved by the SDK for future use
    // (sql, authz). If we got here with one of these, it's an unsupported
    // variant (e.g. wrong method) and should 404 rather than be treated
