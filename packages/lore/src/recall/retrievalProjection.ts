...
export interface UnifiedResultItem {
    ...
    score: number;
    stale_warning?: true;
    /** D1: this node's own raw vector-leg cosine similarity (all phrasings,
     *  max). Additive only — never present for a keyword/traversal-only
     *  match, and `score` above is never derived from it. */
    similarity?: number;
    /** D1: `similarity`'s z-score against the workspace's calibration fit.
     *  Additive only; absent when calibration isn't usable (see `_meta`). */
    relevance?: number;
}
...
function projectNode(
    node: ProjectableNode, matchedBy: MatchKind[], score: number,
    ...
    if (relevance !== undefined) item.relevance = relevance;
    return item;
}
...
export function projectResults(results: RetrievalResult[]): UnifiedResultItem[] {
    return results.map((r) => projectNode(r.node as unknown as ProjectableNode, r.matchedBy, r.score, r.similarity ?? undefined, r.relevance ?? undefined));
}

/** Project an already-scored {node,matchedBy,score} list (e.g. a surface that
 *  merged core + legacy items). `similarity`/`relevance` (D1) are optional —
 *  absent on the legacy workspace:"*" fan-out, which never runs retrieve(). */
export function projectScored(items: Array<{ node: ProjectableNode; matchedBy: MatchKind[]; score: number; similarity?: number | null; relevance?: number | null }>): UnifiedResultItem[] {
    return items.map((s) => projectNode(s.node, s.matchedBy, s.score, s.similarity ?? undefined, s.relevance ?? undefined));
}
